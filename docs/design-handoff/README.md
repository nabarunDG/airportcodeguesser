...
- **Boarding group is now derived from the tier**, not from `max(1, 10 − floor(score/10))` — Million Miler boards Group 1, Standby boards Group 10, and the two change in lockstep. Keeping the old formula after the recalibration would have let the pass print a contradiction like "Middle Seat · GROUP 5".
